...
const Fleets = () => {
    ...
    const columns = ['Fleet Name', 'Type', 'Billing', 'Location', 'Support Email', 'Created Date', 'Action'];
    ...
    const ActionIcons = ({ fleet, onDelete }) => {
        ...
        return (
            <div className='flex'>
                <EditIcon
                    className='cursor-pointer bg-gray-800 text-white p-0.5 rounded-lg mr-1'
                    onClick={() => navigate('edit-fleet', { state: { fleet: fleetData } })}
                />
                <DeleteIcon
                    className='cursor-pointer bg-gray-800 text-white p-0.5 rounded-lg '
                    onClick={onDelete}
                />
            </div>
        );
    ...
    const filteredRows = data?.filter(item => item?.name?.toString().toLowerCase().includes(search.toLowerCase()))
                                        ...
                                        .map((row, index) => {
                                            return (
                                                <TableRow hover role="checkbox" tabIndex={-1} key={index}>
                                                    <TableCell align="left" className='!py-3'>{row?.name}</TableCell>
                                                    <TableCell align="left" className='!py-1'>{row?.type}</TableCell>
                                                    ...
                                                    <TableCell align="left" className='!py-1'>{row?.location}</TableCell>
                                                    <TableCell align="left" className='!py-1'>{row?.email}</TableCell>
                                                    <TableCell align="left" className='!py-1'>{row?.createdDate}</TableCell>
                                                    <TableCell align="left" className='!py-1'>
                                                        <ActionIcons fleet={row} onDelete={() => setDeleteIndex(index)} />
                                                    </TableCell>
                                                </TableRow>
                                            );
                                        })}
                    ...
                    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black bg-opacity-50">
                        <div className="relative bg-white flex flex-col h-60 w-[28rem] items-center justify-center text-center rounded-xl py-3 px-6">
                            <div className="flex w-full justify-between pb-2">
                                <CloseIcon className="absolute right-3 cursor-pointer" onClick={() => setDeleteIndex(null)} />
